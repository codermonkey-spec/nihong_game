import { Center, Cylinder, Text3D, Sphere } from "@react-three/drei";
import { CylinderCollider, RigidBody } from "@react-three/rapier";
import { Leva, useControls } from 'leva'
import { useGameStore } from "../store";

export const KanaSpots = () => {
  const { level, currentKana, currentStage, mode, kanaTouched } = useGameStore((state) => ({
    level: state.level,
    ...
    >
      ...
      <group position-x={3.5} position-z={-3.5}>
        ...
        <Center position-y={0.8}>
          <Text3D
            font={require('../assets/fonts/Noto Sans JP ExtraBold_Regular.json')}
            size={0.82}
            rotation-y={-(index / level[currentStage].length) * Math.PI * 2}
          >
